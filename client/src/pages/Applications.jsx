        ...
        <div className='flex gap-2 mb-6 mt-3'>
          {
            ...
            <label className='flex items-center' htmlFor="resumeUpload">
              <p className='bg-emerald-100 text-blue-500 px-4 py-2 rounded-lg mr-2'>{resume ? resume.name:"Select Resume"}</p>
              <input onChange={e => setResume(e.target.files[0])} type="file" id='resumeUpload' accept='application/pdf' hidden />
              <img src={assets.profile_upload_icon} alt="" />
            </label>
            <button onClick={updateResume} className='bg-green-100 border border-green-400 rounded-lg px-4 py-2'>Save</button>
            </>
            : <div className='flex gap-2'>
              <a href={userData.resume} target='_blank' className='bg-emerald-100 text-blue-600 px-4 py-2 rounded-lg' >Resume</a>
            ...
            {userApplications.map((job,index) => true ? (
              <tr key={index}>
                ...
                <td className='py-2 px-4 border-b'>
                  <span className={`${job.status === "Accepted" ? "bg-emerald-200" : job.status === "Rejected" ? "bg-red-100" : "bg-blue-100" } px-4 py-1.5 rounded`}>{job.status}</span>
                  </td>
              </tr>
            ) : (null))}
          </tbody>
